...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        // 获取其他数据源配置
        Map<String, LinkedHashMap> dynamicDataSourceProperties = binder.bind("spring.datasource.assets", Map.class).get();
//         遍历从数据源

        dynamicDataSourceProperties.forEach((key, dynamicDataSourceConfig) -> {
            Class<? extends DataSource> dynamicDataSourceclazz = getDataSourceType((String) dynamicDataSourceConfig.get("type"));
            // 绑定参数
            DataSource dynamiceDatasource = bind(dynamicDataSourceclazz, dynamicDataSourceConfig);
            // 获取数据源的key，以便通过该key可以定位到数据源
            customDataSources.put(key, dynamiceDatasource);
